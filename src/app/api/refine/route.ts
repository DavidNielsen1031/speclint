...
2. **problem**: Why this matters — the problem statement (1-2 sentences). Include WHY this matters to the business, not just technically (e.g. business impact, user pain, revenue risk, retention, churn, competitive pressure, or compliance need). If the input mentions customer feedback, support tickets, usage data, or stakeholder requests, preserve that evidence explicitly — it signals demand. End with a concrete "Success Metric:" that states an observable, quantified result with actual numbers — not just what the feature does. Examples: "Success Metric: API returns 200 with valid JSON in < 300ms", "Success Metric: Error count drops to 0 in monitoring within 24h", "Success Metric: User completes flow in < 3 clicks confirmed by QA walkthrough", "Success Metric: Cart abandonment rate drops by 15% within 30 days". Avoid vague outcomes like "users will be able to…" or "the system should…" — those are behaviors, not metrics. If the input is too vague, state what's blocked and quantify the impact (e.g. "team cannot estimate — blocking 3 developers").
  ...
  try {
    ...
    if (!rateCheck.allowed) {
      return NextResponse.json(
        {
          error: 'Daily request limit reached on the free tier (5 requests/day). Upgrade for unlimited requests — Lite at $9/mo, Solo at $29/mo, or Team at $79/mo.',
