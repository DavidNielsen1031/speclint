...
2. **problem**: Why this matters — the problem statement (1-2 sentences). End with a concrete "Success Metric:" that states an observable, quantified result — not just what the feature does. Examples: "Success Metric: API returns 200 with valid JSON in < 300ms", "Success Metric: Error count drops to 0 in monitoring within 24h", "Success Metric: User completes flow in < 3 clicks confirmed by QA walkthrough". Avoid vague outcomes like "users will be able to…" or "the system should…" — those are behaviors, not metrics. If the input is too vague, state what's blocked and quantify the impact (e.g. "team cannot estimate — blocking 3 developers").
